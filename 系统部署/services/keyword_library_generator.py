...
## 用户需求类型识别与正确处理方式（强制执行）

**【识别逻辑】**：关键词生成时，LLM需要识别用户需求属于哪种类型，并采用正确的生成方式。

### 类型一：症状/问题型需求
**特征**：用户遇到某种问题/症状/异常
**常见关键词**：XX异常、XX问题、XX不舒服、XX不XX
**错误做法**：症状+业务拼接 → "XX问题+业务名称"
**正确做法**：用户视角的问题词 → "遇到XX问题怎么办"、"XX问题怎么解决"

| 错误示例 | 正确示例 |
|---------|---------|
| 消化异常+业务 | "遇到消化问题怎么办"、"肠胃不好怎么调理" |
| 失眠+服务 | "经常失眠怎么办"、"睡不着怎么改善" |
| 过敏+产品 | "过敏体质用什么好"、"过敏了怎么护理" |

### 类型二：效果/目标型需求
**特征**：用户想要达到某种效果/目标
**常见关键词**：XX好、XX快、XX高、XX多、长XX、追XX
**错误做法**：效果+业务拼接 → "XX好+业务名称"
**正确做法**：用户视角的目标词 → "想XX怎么办"、"怎么才能XX"

| 错误示例 | 正确示例 |
|---------|---------|
| 长高+产品 | "想让孩子长高怎么办"、"吃什么能长高" |
| 变白+产品 | "皮肤暗沉怎么改善"、"怎么美白有效" |
| 省心+服务 | "想省心怎么解决"、"什么事最让人操心" |

### 类型三：成分/特点型需求
**特征**：用户关注某种成分/特点/材质
**常见关键词**：XX成分、XX材质、XX含量、有XX、无XX
**错误做法**：成分+业务拼接 → "XX成分+业务名称"
**正确做法**：成分选择/对比词 → "XX成分有什么用"、"怎么选含XX的"

| 错误示例 | 正确示例 |
|---------|---------|
| DHA+产品 | "DHA有什么用"、"怎么选DHA含量高的" |
| 有机+产品 | "有机产品为什么贵"、"有机和普通的区别" |
| 进口+产品 | "进口产品怎么辨别"、"进口和国产哪个好" |

### 类型四：人群细分型需求
**特征**：用户属于某种特定人群
**常见关键词**：XX人群、XX体质、XX宝宝、XX人群
**正确做法**：人群+问题/需求 → "XX人群怎么选"、"XX人群适合什么"
...
## 核心原则（必须遵守）
1. **关键词主语必须是用户**：用"用户/我/我们"，不用"产品/业务/服务"
2. **搜索结构是用户问题**：用户搜"遇到XX怎么办"，不搜"XX业务介绍"
3. **禁止拼接**：禁止"症状+业务"、"效果+业务"、"成分+业务"的伪产品词
4. **自动适配**：根据当前业务描述，智能识别需求类型并应用正确方式
