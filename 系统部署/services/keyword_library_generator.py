...
这个阶段用户最大的特点是"还没决定，还在犹豫"。

**核心原则：关键词 = 用户在现实中遇到的具体问题，这个问题本身就圈定了人群**
**严禁**：用核心业务词做前缀拼接（如"志愿填报不靠谱"），这等于什么都没说
**必须**：从用户视角出发，写用户脑子里真正在想的词

关键词类型：
1. **后果担忧型**：用户担心做错选择会有什么后果 → "万一选错了怎么办"
2. **信息缺乏型**：用户不了解某个关键信息 → "300分能上什么学校"
3. **方案纠结型**：用户在多个选项之间犹豫 → "冲好学校还是保稳"
4. **信任障碍型**：用户不信任服务商/产品 → "机构会不会跑路"

**搜前搜关键词生成规则：**
- 从画像的痛点场景出发（pain_points），把每个痛点翻译成用户会搜索的具体问题
- 从画像的顾虑障碍出发（barriers），把每个顾虑翻译成用户担心的具体问题
- 一个画像痛点 = 至少1个搜前搜关键词
- 关键词要能回答：这类人群最担心什么、最常问什么、最容易遇到什么困难

=== 【搜后搜关键词】（25个）===
定义：用户使用/体验之后，遇到的实际操作问题和异常情况。
这个阶段用户最大的特点是"已经买了，在用的时候遇到问题"。

**核心原则：搜后搜 = 用户使用后遇到的具体问题，这个问题本身就圈定了使用场景中遇到困难的人群**
**严禁**：用核心业务词做前缀拼接（如"志愿填报坏了怎么办"）
**必须**：从用户使用后的真实困境出发

**搜后搜关键词类型：**
1. **效果偏差型**：用了之后发现和想象不一样 → "和预期效果差太多"
2. **异常情况型**：使用过程中出现意外问题 → "投档后被退档了怎么办"
3. **售后求助型**：遇到问题不知道找谁 → "志愿填报错了找谁改"
4. **结果判断型**：不知道当前结果算好还是坏 → "这个分数能被录取吗"

**搜后搜关键词生成规则：**
- 从画像的痛点场景出发（pain_points），推导使用后可能遇到的后续问题
- 从画像的使用场景出发（pain_scenarios），推导在具体场景中容易出现的问题
- 一个使用场景痛点 = 至少1个搜后搜关键词
- 关键词要能回答：用了之后最担心发生什么、最常遇到什么问题、最难解决什么

=== 【信任佐证关键词】（15个）===
解决"能不能帮我做好/买好"的问题。

**核心原则：信任佐证 = 用户在问"能不能帮我做好"时的具体信任障碍**
**严禁**：用核心业务词做前缀拼接（如"志愿填报22年老店"）
**必须**：从用户的信任顾虑出发

**信任佐证关键词类型：**
1. **售后担忧型**：担心出问题找不到人 → "不满意能找到人处理吗"
2. **结果怀疑型**：怀疑效果是否真实 → "好评是不是刷的"
3. **资质验证型**：想确认对方是否有能力 → "有没有和我情况类似的案例"
4. **风险规避型**：担心损失无法挽回 → "万一不靠谱钱能退吗"

**信任佐证关键词生成规则：**
- 从画像的顾虑障碍出发（barriers），把每个顾虑翻译成用户会搜索的具体信任问题
- 画像用户最担心什么，就生成什么类型的信任佐证词
...
**核心原则：关键词 = 购买者（家长）在选购时遇到的具体问题，这个问题本身就圈定了人群**
**严禁**：用产品名做前缀拼接（如"奶粉不靠谱怎么办"），这等于什么都没说
**必须**：从家长的真实困惑出发

**搜前搜关键词类型（结合画像痛点）：**
1. **体质适配型**：家长不清楚自家孩子的情况适合什么 → "过敏体质喝什么奶粉"
2. **信息缺乏型**：家长不了解某个关键信息 → "成分表怎么看"
3. **方案纠结型**：家长在多个选项之间犹豫 → "水解奶粉和普通奶粉哪个好"
4. **信任障碍型**：家长不信任产品/渠道 → "母婴店推荐的敢不敢信"

**搜前搜关键词生成规则：**
- 从画像的痛点场景出发（pain_points），把每个痛点翻译成家长会搜索的具体问题
- 从画像的顾虑障碍出发（barriers），把每个顾虑翻译成家长担心的具体问题
- 一个画像痛点 = 至少1个搜前搜关键词
...
**核心原则：搜后搜 = 孩子使用后，家长遇到的具体问题**
**严禁**：用产品名做前缀拼接
**必须**：从孩子使用后的真实困境出发

**搜后搜关键词类型（结合画像痛点）：**
1. **效果偏差型**：孩子用了之后发现和想象不一样 → "换了奶粉还是拉肚子怎么回事"
2. **异常情况型**：孩子使用过程中出现意外问题 → "出现湿疹是奶粉原因吗"
3. **售后求助型**：家长遇到问题不知道找谁 → "奶粉问题找店家有用吗"
4. **结果判断型**：家长不知道当前结果算好还是坏 → "大便有奶瓣正常吗"

**搜后搜关键词生成规则：**
- 从画像的痛点场景出发（pain_points），推导孩子使用后家长可能遇到的后续问题
- 一个使用场景痛点 = 至少1个搜后搜关键词
