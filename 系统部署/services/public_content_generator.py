...
def predict_search_behavior_with_industry_scene(
    ...
    Args:
        problem_base: 需求底盘（前置观望种草盘/刚需痛点盘/使用配套搜后种草盘）
        problem_type: 问题类型
        description: 问题描述
        industry_scene: 行业场景类型
    ...
    """
    # 获取需求底盘基础预判
    base_behavior = predict_search_behavior(problem_base, problem_type, description)
...
# 【搜索行为预判字段 - 画像生成时新增】
# =============================================================================
# 基于 problem_base（需求底盘）映射搜索行为
SEARCH_BEHAVIOR_PREDICTION_HINTS = {
    PROBLEM_BASE_SOUQIAN_ZHONGCAO: [  # 前置观望种草盘
        "会搜索基础科普类内容",
        "会搜索同类产品对比",
        "会搜索品牌/商家口碑评价",
        "会搜索选购攻略和避坑指南",
    ...
    PROBLEM_BASE_GANGXU_TONGDIAN: [  # 刚需痛点盘
        "会搜索具体产品/服务参数",
        "会搜索真实用户评价和体验",
        "会搜索价格区间和优惠信息",
        "会搜索选购建议和推荐",
    ...
    PROBLEM_BASE_SHIYONG_PEITAO: [  # 使用配套搜后种草盘
        "会搜索使用方法和技巧",
        "会搜索保养维护知识",
        "会搜索配套工具和耗材推荐",
        "会搜索问题解决和应急处理",
...
def predict_search_behavior(problem_base: str, problem_type: str, description: str) -> List[str]:
    """
    ...
    Returns:
        搜索行为预判列表
    """
    base_hints = SEARCH_BEHAVIOR_PREDICTION_HINTS.get(problem_base, SEARCH_BEHAVIOR_PREDICTION_HINTS.get(PROBLEM_BASE_SOUQIAN_ZHONGCAO, []))
...
def mine_problems_and_generate_personas(params: Dict[str, Any]) -> Dict:
    ...
    # =============================================================================
    # 【P0-1优化】前置扫描层：轻量级情绪+体征扫描，无需LLM调用
    # =============================================================================
    def _build_lightweight_emotion_context(biz_desc: str, biz_type: str, svc_scenario: str) -> str:
        """轻量级情绪动因上下文：通过关键词扫描识别情绪类型"""
        emotion_text = (biz_desc or '').lower()
        detected = []
        ...
        scene_map = {'office_enterprise': '职场', 'hotel_restaurant': '餐饮', 'retail_chain': '零售',
                     'institutional': '机构', 'renovation': '装修', 'residential': '家庭', 'other': '通用'}
        scene_name = scene_map.get(svc_scenario, '通用')
        logger.info("[前置扫描] 情绪动因扫描: %s", detected)
        return f"""
=== 【情绪动因识别层】（若无情绪词则跳过）===
识别到情绪类型：{', '.join(detected)}
场景背景：{scene_name}

【情绪类问题生成指引】：
- 问题类型：情绪疑问/心态误区/减压疑问/内耗疑问
- severity：仅影响心情→中；已影响生活/健康→高
- 底盘：焦虑疑惑→前置观望种草盘；影响生活→刚需痛点盘；疏导干货→使用配套搜后种草盘
- 蓝海：有细分人群/特定场景/情绪动机驱动 → blue_ocean
"""

    def _build_lightweight_chronic_context(biz_desc: str, biz_type: str, svc_scenario: str) -> str:
        """轻量级行为场景体征上下文：B2B客户跳过"""
        needs_chronic = biz_type in ('product', 'local_service', 'personal')
        if not needs_chronic:
            logger.info("[前置扫描] 行为场景扫描: B2B跳过")
            return ""
        logger.info("[前置扫描] 行为场景扫描: 终端消费者，启用")
        return f"""
=== 【行为场景体征识别层】（B端企业客户跳过）===
【慢性症状问题生成指引】：
- 问题类型：长期习惯疑问/场景预防误区/姿势疑问/养护时机/早期信号
- 底盘：长期日常不适→前置观望种草盘；急性发作→刚需痛点盘；日常养护→使用配套搜后种草盘
- 蓝海：细分人群/特定场景/慢性累积/预防性需求 → blue_ocean
"""

    emotion_context = _build_lightweight_emotion_context(business_desc, business_type, service_scenario)
    chronic_context = _build_lightweight_chronic_context(business_desc, business_type, service_scenario)

    logger.info("[mine_problems_and_generate_personas] 前置扫描完成: 情绪上下文=%d字, 体征上下文=%d字",
                 len(emotion_context), len(chronic_context))
    # =============================================================================
    # 【前置扫描层结束】
    # =============================================================================
...
- **【必填】每个画像必须包含「搜索行为预判」字段**：锚定用户真实搜索词方向，描述用户会搜索哪些相关内容（必须与问题类型和需求底盘对齐，不是泛化的搜索词）
    ...
    try:
        ...
        if result:
            ...
            for p in portraits:
                ...
                # 强制使用系统识别的 industry_scene（忽略 LLM 返回值，确保使用固定值）
                p['industry_scene'] = industry_scene
                
                # 调试日志
                logger.info(f"[generate_portraits] 画像 {p.get('name', 'unknown')} 设置 industry_scene={industry_scene}")

                # 强制使用系统识别的 trust_demand（基于正确的 industry_scene 生成）
                ...
                # 自动补齐 pain_point_level（痛点级别）
                if 'pain_point_level' not in p or not p['pain_point_level']:
                    # 根据 severity（严重程度）推断痛点级别
                    severity = problem.get('severity', '中')
                    p['pain_point_level'] = 'high' if severity == '高' else ('low' if severity == '低' else 'medium')

                # 自动补齐 decision_stage（决策阶段）
                if 'decision_stage' not in p or not p['decision_stage']:
                    # 根据需求底盘推断决策阶段
                    if problem_base == PROBLEM_BASE_SOUQIAN_ZHONGCAO:
                        p['decision_stage'] = 'awareness'
                    elif problem_base == PROBLEM_BASE_GANGXU_TONGDIAN:
                        p['decision_stage'] = 'consideration'
                    elif problem_base == PROBLEM_BASE_SHIYONG_PEITAO:
                        p['decision_stage'] = 'decision'
                    else:
                        p['decision_stage'] = 'consideration'
