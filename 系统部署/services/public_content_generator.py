...
- 根据产品特性来思考：谁会买这个东西？为什么买？在哪里买？
- 例如：灌香肠 → 过年置办年货的家庭、婚丧嫁娶操办宴席的村民、早餐店/餐馆老板
- 例如：矿泉水定制 → 企业行政、酒店餐厅、婚庆策划、会议活动组织者
- 「买的人≠用的人」场景（如奶粉/纸尿裤）：只输出购买者身份，使用者细节下一步再写
- 同一身份名称在列表中**只出现一次**
    ...
    def _build_llm_dimension_pool(cls, params: Dict[str, Any]) -> str:
        ...
        if not buyer_user_relation_line:
            buyer_user_relation_line = (
                ...
                '  C端**必填**，B端可空\n'
                '  买即用（自用）：买的人就是用的人（如老年人买羊奶自己喝）\n'
                '  买给家人/长辈/孩子：买的人和用的人不同（如宝妈买奶粉给宝宝喝）\n'
        ...
        if cls._desc_has_electronics_service(business_desc) and not cls._desc_has_baby_business(business_desc):
            prelude_head = (
                '【当前业务】手机/数码维修或售卖点：**不要**使用下列「转奶/奶粉」母婴举例；请用「怕偷换件、报价不清、急用、资料怕丢」类过程状态，'
                '与「当场修好、报价透明、用得放心」类结果目标。\n\n'
            )
        ...
        prelude = (
            ...
            '    例（母婴类业务）：「孩子转奶期不知道怎么选奶粉，非常迷茫」\n'
            '    例（买即用/本地配送类）：「不知道哪家水质好、送水快、价格实在」\n'
            '    例（通用）：「信息太多不敢下手，担心买错」\n'
            '  · 目标（goal）= **解决状态后希望达成的终点结果**——\n'
            '    例（母婴）：「孩子健康发育」\n'
            '    例（本地配送/服务）：「喝水放心、送水及时、用得省心」\n'
            '    例（通用）：「买得更放心/买得更值」\n'
            ...
            '  - 买即用（自用）：买的人就是用的人（如办公室桶装水自己喝）。\n'
            '  - 买给家人/长辈/孩子：买的人和用的人不同（如买桶装水给家里老人喝）。\n'
            '  - 送礼/代购：买来送人，不自己用。\n'
