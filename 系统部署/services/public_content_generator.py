...
1. **谁是使用者？**
   - 买即用业务：购买者就是使用者（本人）
   - 买用分离业务：使用者是宝宝/老人/宠物等，购买者≠使用者

2. **buyer_user_relation 字段**：
   - 买即用：填「自用」
   - 买用分离：填「买给长辈」「买给0-3岁宝宝」「买给宠物」等
...
2. 【痛点状态 + 目标】
   - pain_point_commonality = 使用者**当前正处于的焦虑/迷茫/恐惧过程性状态**
   - goal = 使用者解决上述状态后**希望达成的终点结果**
   - 两者是「过程」→「结果」关系，语义必须能串联

3. 【逐字来自维度库】所有维度标签必须**逐字**来自上方「可用维度库」，禁止自造。

4. 【语义一致】维度组合要语义通顺，禁止矛盾搭配。

5. 【C端禁用词】description/痛点/目标中禁止：融资、创始人、CEO、年营收、GMV、拓客、涨销量 等卖家向词汇。

=== 输出格式（只返回JSON数组，5个对象，不要其他文字）===
{{
    "name": "细分人群简称（≤6字），由LLM根据业务推导，禁止空洞概括（如「消费者」「客户」），禁止与 occupation 重复",
    
    "description": "**【从维度库组合，使用者视角】**\n\n格式：使用者年龄段 + 地域 + 消费阶段 + 痛点状态（过程性）+ 目标（终点结果） + 身份（最后）\n\n例（买即用）：\n「25-30岁县城居民，上班族白领，手机突然碎屏又怕资料丢失，想找报价透明、当场能修好的维修店，担心被坑的上班族」\n\n例（买用分离）：\n「有1-3岁宝宝的家庭，宝宝正在断奶期，担心奶粉成分不安全、怕买到劣质品，希望找到安全可靠的婴儿奶粉，焦虑的宝妈」",
    
    "pain_point_commonality": "来自维度库原文。过程性状态，禁止填终点结果",
    "goal": "来自维度库原文。终点结果，禁止填过程状态",
    "buyer_user_relation": "买即用填「自用」；买用分离填「买给长辈」「买给0-3岁宝宝」等",
    "age_range": "来自维度库 age_group 原文（使用者的年龄段）",
    "geo_tag": "跨区域业务填写空字符串 \"\"；本地业务来自维度库 region 原文",
    "consumption_stage": "来自维度库 consumption_stage 原文（使用者所处阶段）",
    "occupation": "来自维度库 job_role 原文",
    "pain_point": "过程状态 → 终点结果",
    "needs": "2-3条需求，分号分隔",
