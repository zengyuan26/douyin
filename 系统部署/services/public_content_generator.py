...
- user_problem_types（使用层）：前置症状+后置体验（拉肚子/过敏/故障 + 不方便/不舒服）
- buyer_concern_types（决策层）：付费顾虑（怕买贵/怕假货/怕不值）
- 示例：奶粉 → user=宝宝拉肚子/过敏；buyer=宝妈怕买到假货"""
    elif business_type == 'local_service':
        buyer_user_hint = """【三层痛点归类规则】
- user_problem_types（使用层）：前置症状+后置体验（故障/损耗 + 不方便/影响生意）
- buyer_concern_types（决策层）：付费顾虑（怕被宰/怕效果差）
- 示例：维修 → user=设备故障/影响生产；buyer=老板怕花钱被宰"""
    elif business_type == 'personal':
        buyer_user_hint = """【三层痛点归类规则】
- user_problem_types（使用层）：前置症状+后置体验（流量差/效果差 + 焦虑/迷茫）
- buyer_concern_types（决策层）：付费顾虑（怕投入打水漂）
- 示例：博主 → user=播放量低/涨粉慢；buyer=博主怕买课被骗"""
    elif business_type == 'enterprise':
        buyer_user_hint = """【三层痛点归类规则】
- user_problem_types（使用层）：前置症状+后置体验（故障/损耗 + 不方便/影响工作）
- buyer_concern_types（决策层）：付费/成本顾虑（怕超预算/怕质量不稳定/怕回本慢）
- 对接层：流程痛点（审批繁琐/沟通成本高）
- 示例：企业采购 → user=员工使用不便；buyer=老板怕超预算；mediator=行政对接麻烦"""
...
{few_shot_section}
=== 问题类型精细化标准（必须严格遵守） ===
【核心原则】问题类型必须按「问题大类 → 细分问题类型」两级划分，禁止使用"使用体验""效果不好"等宽泛表述。

**一、问题大类框架（必须从以下五类中选择）**
1. 功能/效果问题：产品/服务的核心作用无法实现或不达预期
2. 体验/使用问题：操作复杂或使用过程中产生的不适感
3. 安全/质量问题：存在健康/财产风险或产品本身有工艺/材质缺陷
4. 成本/效率问题：使用/采购成本过高或耗时/周转慢
5. 适配/兼容问题：产品/服务与使用场景或目标人群不匹配

**二、细分问题类型要求**
- 每个细分问题类型必须从上述五类中选择对应的大类
- 细分类型必须具体（如"肠道问题""过敏问题""修复效果问题"），禁止"使用体验""效果不好"
- 每个细分类型对应 3-5 个具体表现（用户可感知的症状/场景），用顿号分隔

**三、严重程度分布（仅保留三档，禁止⭐及以下）**
- ⭐⭐⭐⭐⭐（极高）：安全隐患、核心功能完全失效（如宝宝过敏性休克、修复后再次破损）
- ⭐⭐⭐⭐（高）：核心功能不达预期、影响正常使用（如不长肉、体重增长慢、修复成本过高）
- ⭐⭐⭐（中）：非核心体验问题、可通过调整改善（如口感略淡、包装一般）
- ⭐⭐及以下：禁止出现（核心问题清单聚焦高优先级痛点）

**四、行业适配示例（参考格式，勿照抄行业词）**
奶粉行业：
  - 体验/使用问题 → 肠道问题（拉肚子、腹胀、便秘、大便奶瓣）⭐⭐⭐⭐⭐
  - 体验/使用问题 → 过敏问题（牛奶蛋白过敏、乳糖不耐受、皮肤红疹）⭐⭐⭐⭐⭐
  - 功能/效果问题 → 发育问题（不长肉、不长个、体重增长慢）⭐⭐⭐⭐
  - 体验/使用问题 → 口味问题（不爱喝、厌奶、挑食）⭐⭐⭐⭐
餐具修复行业：
  - 功能/效果问题 → 修复效果问题（修复处掉漆、易破损、光泽不一致）⭐⭐⭐⭐⭐
  - 成本/效率问题 → 成本问题（单次修复费用高、修复后使用周期短）⭐⭐⭐⭐
灌香肠行业：
  - 功能/效果问题 → 口感问题（偏硬、偏咸、肉柴、有腥味）⭐⭐⭐⭐⭐
  - 安全/质量问题 → 食材问题（肉不新鲜、添加剂过多、保质期短）⭐⭐⭐⭐⭐
...
- 成本顾虑：年预算、月消耗、采购成本
- 风险顾虑：怕质量不稳定、怕断供、怕售后无保障
- 资质顾虑：怕不专业、怕没资质、怕出问题担责
- 回本顾虑：多久回本、投入产出比
- 注意：不是使用体验，而是出钱决策时的心理障碍
