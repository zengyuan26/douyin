...
5. 【买用关系·C端必填】C端业务必须在 description 或 buyer_user_relation 字段体现「买的人」和「用的人」关系。
   - 买即用（餐饮/维修/理发等）：买的人就是用的人，buyer_user_relation 填「自用」
   - 代买（奶粉/老人用品等）：买的人≠用的人，buyer_user_relation 填「买给长辈/宝宝/家人」
...
{{
    ...
    "description": "**买即用时**：痛点状态（过程性）+ 目标（终点结果）+ 身份（最后）。例：「手机碎屏又怕资料丢失，想找报价清楚、当场能修好、用得放心的店，担心被坑的上班族」。\n**买用分离时**：使用者状态（过程性）+ 购买者担忧 + 身份（最后）。例：「2个月的宝宝乳糖不耐受，喝的奶粉天天拉肚子，不知道该怎么选择，信息太多害怕买错，宝宝受罪的宝妈」。",
