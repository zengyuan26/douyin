...
【思考路径】（分三步）
1. 先想：这个产品/服务解决什么问题？（不是卖什么，是解决什么痛点）
2. 再问：谁会有这个问题？（从问题反推人群）
3. 最后想：这个人群在哪里找？（到他们的场景中找人）

【示例】
- 灌香肠 → 问题：过年要准备年货/办宴席 → 人群：置办年货的家庭、办宴席的主家 → 场景：菜市场、超市
- 矿泉水定制 → 问题：接待/会议需要好水 → 人群：企业行政、会议组织者 → 场景：写字楼、展会
...
【在身份推导中的应用】
- 卖桶装水的 → 思考"企业饮水解决方案"需要找谁 → 企业行政、办公室负责人
- 卖电子秤校准的 → 思考"计量准确保障"需要找谁 → 菜市场摊贩、快递驿站、药店
...
| 业务类型 | 买的人=用的人 | 买的人≠用的人 |
|---------|-------------|--------------|
| 买即用 | 餐饮、维修、理发、手机、桶装水 | - |
| 买用分离 | - | 奶粉、纸尿裤、老人用品、宠物用品 |
| 混合型 | 部分人群自用 | 部分人群代买 |

**买用分离业务示例：**
- 婴儿奶粉 → 宝宝是使用者 → 买用分离 → 人群是「宝妈」「奶爸」「长辈」
- 老人手机 → 老人是使用者 → 买用分离 → 人群是「子女」「晚辈」
- 宠物食品 → 宠物是使用者 → 买用分离 → 人群是「宠物主人」

**买即用业务示例：**
- 手机维修 → 自己手机坏了 → 买即用 → 人群是「手机用户」「上班族」
- 桶装水配送 → 自己喝水 → 买即用 → 人群是「家庭用户」「企业员工」

【本阶段只做「身份标签」】用户点击后将进入下一步生成详细画像；**不要**在本阶段输出使用者年龄、转奶期、健康目标等细节，那些由下一步结合业务描述再生成。

核心原则：只识别**购买/决策侧**的稳定身份称呼，根据实际业务来推断。
...
【核心逻辑】先发现问题 → 人群自动出现 → 提供解决方案
- 水管漏水 → 业主才需要修水管
- 秤计量不准 → 摊贩/商家才需要校准服务
- 要泡好茶 → 爱茶人士才需要好水

【思考路径】（分三步）
1. 这个业务解决什么问题？（不是卖什么，是解决什么痛点）
2. 谁会有这个问题？（从问题反推人群身份）
3. 这个人群在哪里？（场景、地点、行为特征）

=== 🔄 思维方式：产品服务化 ===

【核心理念】卖产品不如卖服务，产品只是服务的载体
- 卖桶装水的 → 不是卖水，是卖"喝水放心的解决方案"
- 卖电子秤校准的 → 不是校准秤，是卖"计量准确的保障"

【推导身份时应用】先想"我这个解决方案帮谁解决了什么问题"，再推导人群：
- 计量准确保障 → 菜市场摊贩（怕缺斤少两被投诉）、快递驿站（怕罚款）、药店（怕检查）
- 喝水放心方案 → 泡茶爱好者（讲究口感）、企业行政（接待客户）、孕婴家庭（安全健康）
...
=== 通用约束 ===
1. 【身份推导·核心】5条画像的 name 字段应体现**不同的细分人群身份**，由你根据业务描述自由推导（不限于预设列表）。例如：
   - 手机维修店 → 可推导「附近居民」「上班族」「学生」「老人/长辈家属」「手机重度依赖者」等
   - 母婴用品店 → 可推导「新晋宝妈」「备孕期女性」「二胎妈妈」「家中长辈」等
   - 桶装水配送 → 可推导「写字楼行政」「家庭用户」「小微商户」「健身爱好者」等
   每条 name（≤6字）须对应一个**真实、可触达**的细分身份，禁止空洞概括（如「消费者」「客户」）。

2. 【差异化·强制】5条之间**至少在3个维度上有明显不同**，可组合维度包括：
   - 身份细分（不同角色：宝妈 vs 宝爸 vs 家中长辈）
   - 年龄段（如25-30岁 vs 30-40岁 vs 50-60岁）
   - 地域（如县城居民 vs 城区核心街道 vs 郊区住户）
   - 消费阶段（如备孕期 vs 有0-1岁宝宝 vs 空巢期）
   - 买用关系（买给自己 vs 买给长辈 vs 买给孩子）
   - 职业/场景（如上班族白领 vs 个体老板 vs 退休人员）
   **禁止**仅靠换一个年龄段凑数，必须同时换至少1个其他维度使画像语义不同。

3. 【痛点状态 + 目标·必填】
  **概念区分（必须理解后再写）：**
  · 痛点状态（pain_point_commonality）= 此人**当前正处于的焦虑/迷茫/恐惧过程性状态**——
    例（本地服务）：「急用手机但不知道哪家维修靠谱，怕被坑」
    例（买即用）：「工作忙没时间，不知道哪家餐厅干净又好吃」
    例（代买场景）：「孩子转奶期不知道怎么选，非常迷茫」
  · 目标（goal）= 解决上述状态后**希望达成的终点结果**——
    例：「当场修好、报价透明、用得放心」
    例：「省心省力、吃得好」
    例：「孩子健康成长」
  两者是「过程」→「结果」关系，语义必须能串联。**禁止**在痛点状态或目标上搞差异化（本批统一）。
4. 【地域约束】{region_constraint}
5. 【买用关系·C端必填·必须先判断业务类型】**必须先判断「买的人」和「用的人」是否相同，再选择填写：**

   **买用分离业务（买的人≠用的人）**：buyer_user_relation 填「买给0-3岁宝宝」「买给长辈」「买给宠物」等
   - 典型业务：奶粉、纸尿裤、婴儿推车、宝宝辅食、老人保健品、宠物用品
   - 判断依据：产品/服务是给别人用的，购买者不是使用者
   - 画像描述格式：「使用者状态/问题 + 购买者担忧 + 身份」

   **买即用业务（买的人=用的人）**：buyer_user_relation 填「自用」
   - 典型业务：餐饮、维修、理发、桶装水、手机、自用食品
   - 判断依据：买的人就是用的人，自己消费自己使用
   - 画像描述格式：「本人痛点 + 本人目标 + 身份」

   **重要**：不要只看业务名称，要看使用场景！
   - 「婴儿奶粉」→ 宝宝喝 → 买用分离
   - 「成人奶粉」→ 自己喝 → 买即用
   - 「老人手机」→ 老人用 → 买用分离（子女买给老人）
   - 「手机贴膜」→ 自己贴 → 买即用

6. 【⚠️买用分离·维度集中于使用者】**仅当**业务本身为「代买给不同使用者」的商品时才适用（如奶粉、老人用品、宠物用品）。
   **手机维修、餐饮、理发等买即用本地服务不适用本条**，买者即用者，维度围绕本人，禁止硬套奶粉话术。
7. 【逐字来自维度库】所有维度标签（年龄段、地域、消费阶段、痛点共性、目标、买用关系等）必须**逐字**来自上方「可用维度库」，禁止自造。
8. 【语义一致】维度组合要语义通顺，禁止矛盾搭配（如「县城居民」+「跨省配送」）。
9. 【C端禁用词】description/痛点/目标中禁止：融资、天使轮、创始人、CEO、总监、团队规模、年营收、GMV、拓客、涨销量 等卖家或企业向词汇。
10. 【痛点与目标禁词】pain_point_commonality、goal 两字段**额外禁止**出现：遇到瓶颈、想要增长、想要转型、寻求突破、规模化、营收——应改用消费者语言（如不知道选哪个、担心成分、希望买得放心）。

=== 输出格式（只返回JSON数组，5个对象，不要其他文字）===
{{
    "name": "细分人群简称（≤6字），由LLM根据业务推导，禁止空洞概括（如「消费者」「客户」），禁止与 occupation 重复",
    "description": "**【必须先判断买用关系再写】**\n\n**买即用时**：本人痛点（过程性）+ 本人目标（终点结果）+ 身份。\n例：「工作忙没时间做饭，想找干净又快的外卖，不知道哪家卫生，吃坏肚子很麻烦的上班族」。\n\n**买用分离时**：使用者状态/问题 + 购买者担忧 + 购买者身份。\n例：「2个月宝宝乳糖不耐受，喝普通奶粉天天拉肚子，不知道哪款奶粉安全有效，害怕买错宝宝受罪的焦虑宝妈」。",
    "pain_point_commonality": "**痛点状态（过程性）**：来自维度库原文。指此人**当前正处于的焦虑/迷茫/恐惧状态**——如「孩子转奶期不知道选哪个，正在多方比较、非常迷茫」（这是过程状态）；禁止填终点结果（如「孩子健康发育」）",
    "goal": "**目标（终点结果）**：来自维度库原文。若用户指定了使用者目标（user_goal），则以此为准；否则从库中选择。指解决痛点状态后**希望达成的结果**——如「孩子健康发育」「买得更放心」「买得更值」（这是终点结果）；禁止填过程状态（如「不知道该怎么选」）",
    "buyer_user_relation": "C端必填。买即用时填「自用」；买用分离时填「买给0-3岁宝宝」「买给长辈」「买给宠物」等",
    "age_range": "来自维度库 age_group 原文",
    "geo_tag": "跨区域业务填写空字符串 \"\"；本地业务来自维度库 region 原文（如「县城居民」「同城1小时配送圈」）",
    "consumption_stage": "来自维度库 consumption_stage 原文（如「备孕期」「有0-1岁宝宝」「空巢期」）",
    "occupation": "来自维度库 job_role 原文",
    "pain_point": "一句完整的话：**先说当前正处于的过程性焦虑/迷茫状态**（因痛点状态）+ **再说解决后想达成的终点结果**（因目标）。例：「孩子转奶期不知道怎么选，非常焦虑 → 希望找到适合的方案，孩子健康成长」/ 「不知道成分是否安全，不敢下手 → 希望确认品质，买得放心」",
    "needs": "2-3条需求，分号分隔",
