...
{{
    ...
    "user_problem_types": [
        {{"identity": "核心人群A", "problem_base": "刚需痛点", "problem_category": "身体/消化异常", "problem_type": "消化/肠胃反应", "display_name": "使用后肠胃不适", "description": "具体肠胃症状表现（如腹胀、腹泻、便秘、反胃等）", "severity": "高", "scenarios": ["初次使用", "换新款", "季节变化"], "market_type": "blue_ocean", "market_reason": "细分人群", "problem_keywords": [{{"keyword": "{product_name}用完拉肚子怎么回事", "type": "blue_ocean", "source": "场景痛点词"}}, {{"keyword": "换了{product_name}后腹胀便秘正常吗", "type": "blue_ocean", "source": "长尾转化词"}}], "content_direction": "转化型"}},
...
[L_BRACE]
    "portraits": [
        [L_BRACE]
            "name": "画像名称（可加入场景关键词）",
            "portrait_summary": "【必填】2～3句口语化自然中文。结构公式：身份 + 当前问题/症状 + 想转变（解决问题）+ 受限于困境 + 情绪。禁止用【】、禁止列模板标签、禁止JSON式字段名。",
            "使用场景": "该画像对应的具体使用场景",
...
1. **portrait_summary 结构（五要素，必须具体）**：
   ...
   - 情绪：具体情绪词（如：急得睡不着；看着家人难受心里着急；担心选错了浪费钱更耽误事）

2. **禁止抽象表述**：
   - ✗ 禁止："担心质量问题" → ✓ 改为："怕买到质量不稳定的产品用完出状况"
   - ✗ 禁止："价格太贵" → ✓ 改为："比同类产品贵不少，预算有限怕买亏了"
   - ✗ 禁止："不知道怎么选" → ✓ 改为："配料表上20多种成分，不知道哪个是好的"
   - ✗ 禁止："产品效果不好" → ✓ 改为："使用后效果不明显、不舒适"
