...
5. **如果是定制礼赠业务**，必须包含"办宴体面"、"送礼走心"、"怕定制踩坑丢面子"等心理关键词
...
def build_v2_portrait_context(business_description: str = '') -> dict:
    """
    构建V2画像生成的完整上下文
    从画像维度服务获取精简维度信息
    """
...
目标用户：{input_data.get('target_users', '')}"""
    ...
    # 获取画像维度上下文
    portrait_context = build_v2_portrait_context(input_data.get('description', ''))
    ...
    for problem in problems:
        prompt = PORTRAIT_V2_PROMPT.format(
            ...
            custom_gift_section=portrait_context.get('custom_gift_section', '')
        )
